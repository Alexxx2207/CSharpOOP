...
namespace Stealer
{
    public class Spy
    {
        public string CollectGettersAndSetters(string className)
        {
            ...
            MethodInfo[] gettersmethodInfos = classType.GetMethods(BindingFlags.Public|BindingFlags.Instance|BindingFlags.Static|BindingFlags.NonPublic).Where(m => m.Name.StartsWith("get")).ToArray();
            MethodInfo[] settersmethodInfos = classType.GetMethods(BindingFlags.Public | BindingFlags.Instance | BindingFlags.Static | BindingFlags.NonPublic).Where(m => m.Name.StartsWith("set")).ToArray();
            gettersmethodInfos.ToList().ForEach(m => sb.AppendLine($"{m.Name} will return {m.ReturnType}"));
            settersmethodInfos.ToList().ForEach(m => sb.AppendLine($"{m.Name} will set field of {m.GetParameters()[0].ParameterType}"));
